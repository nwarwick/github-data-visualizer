...
module.exports = function profile($, url, callback) {
  ...
  $('.counter').each(function () {
    stats.push(parseInt($(this).text().replace(/^\D/g, ""), 10));
  });
  ...
  data.repositories   = stats[0];
  data.starred        = stats[1]; // number of repositories user has starred
  data.followercount  = stats[2]; // number of people folling this user
  data.followingcount = stats[3]; // number of people this user is following

  // General Info
  //data.worksfor = $('.vcard-detail').first().text();      // Works for
  data.worksfor = $('.octicon-organization').parent().text().trim();
  ...
  data.contribs = parseInt(contribs[1].replace(/^\D/g, ""), 10); // regex removes all non-digits from string
